    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                @endif
                <div class="card-header">新建列表型页面</div>
                <div class="card-body">
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        {{--</div>--}}
                        {{--<div class="form-group">--}}
                            {{--<label>负责人联系方式* <span class="size"></span></label>--}}
                            {{--<input type="text" class="form-control" name="memberphone" value="{{$page->memberphone}}">--}}
                        {{--</div>--}}

                        <div class="form-group">
                            @php
                                $ops = \App\Sidemenu::where('rank','=','1')->get();
                            @endphp
                            <label>内容* <span class="size"></span></label>

                            <select name="kindcontent" id="" class="form-control">
                                <option value="">------------请选择--------------</option>

                                @foreach($ops as $op)
                                    <option value="{{$op->id}}" @if($op->id == $page->kindcontent) selected @endif>{{$op->name}}</option>
                                @endforeach
                                @if(count($ops) > 0)
                                    <option value="-1" @if($page->kindcontent == -1) selected @endif>所有业务</option>
                                @else
                                    <option value="-2" @if($page->kindcontent == -2) selected @endif> 暂无</option>
                                @endif
                            </select>
                        </div>
...
@section('js')
    <script type="text/javascript">
        ...
        ue.ready(function() {
            //设置编辑器的内容
            ue.execCommand("inserthtml",'请编辑你的页面') ;
        });
    </script>
